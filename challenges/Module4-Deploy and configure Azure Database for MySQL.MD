...
1. Simply click the Deploy to Azure button below and follow the wizard to create the resources. You will need to log in to the Azure Portal, Use the lab azure credentials to login.
...
2. Select **existing Resource Group** for deployment. Provide unique name for **Site_name**, any name for **Serverfarms_name** and left other as default value.
3. Check-In **terms and condition** and click on **Purchase.**
...
1. Sign in with your Microsoft Azure Credential.

1. You now have an Azure CLI session open against the Azure Subscription hosting your lab.

1. You will now use the CLI to provision an Azure Database for MySQL. In, open PowerShell  prompt, use the following command to provision an new Azure Database for MySQL:
...
1. First we need to acquire a copy of the code for the website. To do this we will clone it from an existing GitHub repository. In the open PowerShell command prompt run the following commands one by one:
```
    cd \  
    mkdir code  
    cd code  
...
1. In overview section, copy the **Url** and add **scm.** after website name as show below:
```
